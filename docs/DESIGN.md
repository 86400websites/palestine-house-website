...
- **Gated shell:** ⚠️ **superseded by Stage 4 (2026-08-12) — rewritten at PP5.** The workspace v2 chrome is a **fixed top header with five links (About · Setup · Operate · Program · Support)** + tooltips, an account button, a mobile panel, and a moss footer with the search CTA / Ask HQ / tatreez band / Arabic tagline. Its design layer is `src/styles/workspace-v2.css`, entirely scoped to `.pw-root`. The pre-Stage-4 description follows for history: *persistent left sidebar (Welcome · Stages · Managing & Operating · Programming/Live Programming · Academy · Resources · House Applications · Account, Support pinned at bottom); pending state shows items visible but locked (lock icon + "Locked" label, never color alone).*
