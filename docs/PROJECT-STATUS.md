...
| | |
|---|---|
| **Current stage** | **Stage 2 — S1 build complete on its branch.** Next: owner Preview sign-off + merge, then S2 |
| **Active sprint** | S1 — design consistency pass + owner design refresh (branch `claude/sprint-s1-design-consistency`, draft PR open; owner review pending) |
| **Next action** | Owner: review the S1 PR on Vercel Preview (dark sections, white art cards, hero balance, mobile nav one-liners) and merge. Remaining Dependabot action: comment `@dependabot ignore this major version` on #9–#12 (majors = locked-stack upgrades; #6/#7/#8 Actions bumps already merged). Then start **S2 — DB phase 1** (create the non-prod Supabase project first, §4) |
...
| **Last updated** | 2026-06-12 — S1 built in 14 owner-gated sub-steps: 24 white-bg artworks (54.8→22.4 MB), owner background scheme + premium white cards site-wide, hero balance, mega-menu art swap, 2 approved copy changes, support booklet reorder, 1a–1d audits passed (focus-ring AA fix on dark surfaces) |
...
| S1 Design consistency pass (1a–1d) | ✅ | S1 PR (merge pending) | 2026-06-12 | + owner design refresh: white-bg art, dark sections, premium cards, hero balance, nav art swap, 2 copy changes (§4); audits 1a–1d passed |
