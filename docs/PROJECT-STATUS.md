...
| **Next action** | Owner: review the S2 PR and merge (Preview shows only the unchanged site — S2 ships SQL, no UI; `0001`–`0007` already applied + verified on both databases). Then start **S3 — Auth complete** (Supabase `@supabase/ssr` clients + `middleware.ts` + login; Apply live = sign-up in 3c). Supabase env vars for Preview/Dev now point at the test project (`sdszcralogcrujtyghig`); production env vars already point at the prod project. (Dependabot #9–#12 already resolved — no open branches remain.) |
