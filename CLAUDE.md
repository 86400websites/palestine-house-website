...
- **Retired surfaces, and the one table that outlived them.** PP5 deleted the Academy, the Live hub and the checklist UI; `0030` then dropped `academy_modules`, `checklist_items` and `checklist_progress` from production (2026-08-17). **`programming_sessions` is still there** — RLS on, **zero policies, zero rows**, so it is default-deny to everyone. Do not treat an unreachable table as an unprotected one, and do not add a caller, a policy or a row to it without a decision that says why. *(Verified against production 2026-08-21.)*
