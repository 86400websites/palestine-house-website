...
| **Wall 4a** | Behaviour proof — tests | **Built at SYS2** — Playwright harness in `tests/e2e/` (`pnpm run test:e2e`, deployed-Preview target, four auth roles); the first full Launch Gate run + GO verdict is SYS2's exit gate |
...
| Check 5 | Tests run when present | **Present — SYS2**, outside `verify` by design. `test:e2e` needs a **deployed Preview URL** and role credentials, which the PR-time `verify` job does not have (the Vercel deploy races CI) — so the suite runs as the operator-driven launch gate against Previews and as the `morning-check.yml` workflow, never as a `verify` step. `ci.yml` deliberately untouched (D-SYS-4) |
...
**4a — tests (built at SYS2, 2026-08-22).** `@playwright/test` is a dev dependency, the suite lives in `tests/e2e/` (`pnpm run test:e2e`), and it targets a **deployed Vercel Preview** via `PLAYWRIGHT_BASE_URL` with one stored auth session per role (anonymous · pending partner · approved partner · HQ admin — robot accounts in the non-production Supabase project only). One test per approved line of `docs/FEATURE-LIST.md` — including the negative access-gate tests for all four roles — with the GO verdict recorded in `docs/test-reports/`. **No `verify` test step, by design:** the suite needs a deployed Preview URL and credentials that the PR-time CI job does not have, so it runs as the operator-driven launch gate and as the (initially disabled) `morning-check.yml` workflow instead; `ci.yml` stays untouched (D-SYS-4). This wall is *built*; it *holds* from the first full 100%-green run with the owner's GO — SYS2's exit gate.
