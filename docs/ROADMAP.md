...
| **S11 — Admin content management** *(post-launch)* | 11a admin-panel CRUD over the private-page content (the 30 element/topic bodies, academy videos, resources/templates) behind the existing `is_admin()` gate · 11b add / update / delete with RLS-safe writes · *(the old-S10 admin-management UI — add/remove admins — folds in here, to confirm at scope)* | The client fully self-manages all private-page content + resources without scripts — builds the graduated backlog `/admin/content`. Exit: an admin can add/update/delete content end-to-end behind the admin gate; writes RLS-safe; no content loss; versioned up + `.down.sql` + RLS in the PR (TEST-first, `WORKFLOW.md` §14). | S5, S10 |
| **S12 — Email-automation placeholders + settings** *(post-launch)* | 12a Mailchimp: booklet capture (`lead-booklet-a/b`) + newsletter + apply tagging · 12b Resend: contact + support + the **approval/decline email** (deferred from S4) + **declined→contact** path · 12c all settings + env wiring, no-op until keys present; document SPF/DKIM/DMARC verification | Everything wired with honest no-op placeholders so the owner goes live by **adding keys/domain alone** (Mailchimp API key + audience id; Resend env + verified sending domain). Absorbs the old-S10 email + approval/decline email. Exit: each integration no-ops cleanly without env vars; keys server-only; switch-on is env-vars-only. | S3 |
| **S13 — Full testing + fix-all → 0 bugs** *(post-launch)* | 13a multi-hat end-to-end walk (copywriting → UX → feature testing — every page, feature, CTA, link, a11y, security) · 13b **fix everything found, same sprint** · 13c re-verify | Cover everything in testing so there are no later surprises, then fix it all → a final, fully-working version with **0 errors / 0 bugs**. Absorbs the old S11 (journey QA) + S12 (fix-all). Exit: complete severity-ranked pass with every finding fixed + re-verified; production-quality, zero known bugs. | S10, S11, S12 |
| **S14 — Final hardening + relaunch** *(post-launch)* | 14a Upstash rate-limiting + Turnstile on public writes · 14b fail-closed verification in Production; CSP/headers verified live; secret-path threat-model pass · 14c custom domain + SEO · 14d final relaunch | Final hardening + go-live on the **custom domain** with SEO. Absorbs the old S12 hardening + final launch. Exit: full `SECURITY-CHECKLIST.md` §1–§15 pass; domain + SEO live; production re-smoke-tested; relaunched. | S13 |
