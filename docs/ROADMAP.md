...
| **S12 — Email-automation placeholders + settings** *(post-launch)* | 12a Mailchimp: booklet capture (`lead-booklet-a/b`) + newsletter + apply tagging · 12b Resend: contact + support + the **approval/decline email** (deferred from S4) + **declined→contact** path · 12c all settings + env wiring, no-op until keys present; document SPF/DKIM/DMARC verification | Everything wired with honest no-op placeholders so the owner goes live by **adding keys/domain alone** (Mailchimp API key + audience id; Resend env + verified sending domain). Absorbs the old-S10 email + approval/decline email. Exit: each integration no-ops cleanly without env vars; keys server-only; switch-on is env-vars-only. Brief: `docs/sprint-prompts/s12-email-automation-placeholders.md`. | S3 |
