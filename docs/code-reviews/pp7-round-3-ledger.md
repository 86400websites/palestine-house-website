...
| **B4** | The down-generator snapshots a different set than the up-migration deletes. | ⚠️ **CONFIRMED AS LOGIC · NOT CURRENTLY TRIGGERED** | `generate-0030-down.ts:195` and `:204` walk `topics` and collect elements/resources *per topic*. `0030` deletes by `elements.code !~ '^[0-9]'` — every legacy element, topic or no. So an unlinked legacy element (and any resource hanging off it) is deleted and never snapshotted. **Measured on production today: `legacy_elements_without_topic` = 0 and `resources_deleted_but_not_snapshotted` = 0**, so the current file is complete. The defect is a live trap for the *next* generation, not a hole in the file on disk — and the fix is still right, because the generator must snapshot the up-migration's own predicate rather than a proxy for it. **Closed at 7-e** by construction: elements from `admin_list_elements()` (the table, not the topics), topics selected *by element*, groups by `slug not like '%-focus-areas'`, resources walked over every legacy element. Regenerated file: identical shape (10 · 33 · 33 · 297), confirming the two strategies still agree on production. |
...
| **M4** | Restored `resources` rows omit `focus_area_code` and timestamps. | ✅ **CONFIRMED — AND IT BITES TODAY** | `generate-0030-down.ts:314` inserts eleven columns and omits `focus_area_code` and `created_at`; the elements insert at `:280` omits `created_at`/`updated_at`. **Measured on production: all 297 template rows carry a non-null `focus_area_code`**, and the `resources_focus_area_shape` CHECK permits NULL — so a rollback restores 297 rows with the column silently emptied and no error raised. **Sized at 7-e:** the column is legacy-only (0 of the 112 new rows carry one) and nothing in `src/` reads it, so the consequence is a rollback that is not a faithful restore rather than a broken product. Fixed by derivation from the owning element, verified per row against production by digest (`c5472e4f…`). The **timestamp** half is not fixable through the RPC surface — no admin RPC returns `created_at`/`updated_at` — and is documented in the generated file and its README instead, with the measurement that sizes it: nothing in `src/` reads any of the three. |
