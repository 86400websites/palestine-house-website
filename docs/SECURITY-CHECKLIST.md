...
- [ ] 🔴 **Templates & booklets:** the 297 templates live in a **private** Storage bucket; downloads only via **server-issued signed URLs** to authenticated, approved users. (The two public booklet PDFs — *The House Promise*, *Operating Model & Governance* — are the only public files, served as public lead magnets.) **Stage 4 addition (D-PP-f, 2026-08-12):** those 297 rows are now the live per-topic **templates grid**, selected by `element_id` + `doc_key IS NULL` + `code IS NOT NULL`. Any query or RPC that builds the grid **must also require `is_public = false` AND `storage_bucket = 'resources'`** — the three-column predicate alone would admit a future public upload into a partner-only surface (found by review, 2026-08-12).
  - ⚠️ **Half of this is still owed, and PP6 owes it (D-PP-i).** As shipped, the grid enforces `is_public = false` in the app but **cannot** check `storage_bucket`: `get_resources()` does not return that column and `0027` became immutable when production ran it. That is safe today only because **no shipped admin RPC can set `storage_bucket`** — `admin_update_resource` cannot edit `code`, `doc_key` or `storage_path` (0023/0026) — so no wrong-bucket row can exist. **PP6 ships the upload path that can, so PP6's `0029` must ship the guard with it**, in the RPC rather than in the UI. PP5 verified the invariant still holds in production (297 in-grid rows, 0 wrong-bucket) but did not and could not close it.
