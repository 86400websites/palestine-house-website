...
The SOP's Phase 1 requires a **100% green whole-site test run on the release candidate**, with a **GO** verdict recorded in `docs/test-reports/`. This repo has **no `test` script, no `tests/e2e/`, no `docs/testing-setup/` and no `docs/test-reports/`** — none of them exist today, and none existed on 2026-06-19. `package.json` defines exactly five scripts: `dev`, `build`, `start`, `lint`, `typecheck`.
...
- ⛔ **Morning check ready.** The morning-check workflow, its template and the `@morning` tagging all live in `docs/testing-setup/` — which **arrives in SYS2**, added *disabled*, with the selection of the 5–7 tests (and its production-login question) an explicit owner decision at that point.
