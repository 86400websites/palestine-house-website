...
| | |
|---|---|
| **Current stage** | **Stage 2 — S3 (Auth complete) built in owner-gated sub-steps; pushed to the open PR.** S2 merged (#17). Migration `0008` applied + verified on test + prod. Next: owner Preview auth testing + merge, then S4 |
| **Active sprint** | S3 — Auth complete (branch `claude/sprint-s3-auth-complete`, PR open; `@supabase/ssr` clients + `src/middleware.ts` + live login/logout + forgot/update password + Apply-live sign-up; `0008` applied + verified both DBs; owner Preview test + merge pending) |
| **Next action** | Owner: test the auth flows on the Vercel **Preview** (against the test DB `sdszcralogcrujtyghig`): sign up via `/apply` (confirm a `profiles` row with `full_name` + a `pending` `applications` row land), sign in / sign out, forgot → reset email resolves to the **Preview** origin → update password. Then merge the S3 PR and start **S4 — Approval gate + `/admin/approvals`**. (Reminder: turn off "Confirm email" was done on both projects; Apply now collects a password — see §4.) |
...
| **Last updated** | 2026-06-16 — S3 built (3a clients+`middleware.ts`+login/logout · 3b forgot/update password + `/auth/confirm` · 3c Apply live = sign-up + `applications` record). Auth runs server-side (Server Actions / Route Handlers) so CSP `connect-src` stays `'self'`; all public pages stay statically rendered. Migration `0008` (map `full_name` from signup metadata) applied + verified on test + prod. Apply gained an owner-approved Password field (§4) |
