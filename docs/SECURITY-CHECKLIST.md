...
- [ ] 🔴 **Approval enforcement:** every platform data RPC — `get_platform_sections`, `get_platform_topics`, `get_element`, `get_resources` — checks `profiles.is_approved` **server-side**, and every gated page checks approval again before it renders. An unapproved (pending) session can resolve only its own profile/approval status — never a guide body, a topic summary, a template row, or any of the dormant checklist/academy/session data. **The global search index inherits this** rather than restating it: it is built from those same gated reads, so a pending or anonymous caller gets an empty index (D-PP-j), and it carries no resource ids, storage paths or bucket names.
