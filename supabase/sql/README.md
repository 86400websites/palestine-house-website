...
| 0018 | `0018_account_prefs.up.sql` | S6 (6g): account self-management for `/account`. Adds **`profiles.marketing_opt_in`** (boolean, default **false** — privacy-respecting, expand-only) and **`set_my_account(p_display_name, p_marketing_opt_in)`** — the owner-scoped write for the current user's OWN profile. Scoped by `auth.uid()` and deliberately **not** `is_approved()`-gated (account management is available before approval), but it can only ever set `full_name` + `marketing_opt_in` — never `is_approved` — so there is no escalation. Display name trimmed, capped at 120, blank → NULL. Hardened (`SECURITY DEFINER`, `search_path=''`, EXECUTE revoked from public/anon, granted authenticated). |
| 0019 | `0019_support_requests.up.sql` | S6 (6g): the `/support` form's storage + write path. Adds **`support_requests`** (RLS default-deny, **no client policy**) plus **`submit_support_request(p_subject, p_message)`** — an **APPROVED-only** insert with `user_id` forced from `auth.uid()` (the launch anti-abuse posture, §7 / D-S6-a; subject+message required, length-capped) — and **`admin_list_support_requests()`**, the `is_admin()`-gated read for the owner (zero rows for non-admins). Email delivery is deferred to the post-launch email sprint — requests are **stored, not lost**. Both RPCs hardened exactly like the S2/S4 helpers. |
...
- **Migrations** keep a single, ever-increasing number across all sprints. S2 used `0001`–`0007`,
  ...
  (post-review hardening); S6 (private platform pages) added `0017` (resources download), `0018` (account prefs), and `0019` (support requests). The next database sprint continues at `0020`. Never reuse or renumber. Once a migration has been applied to production it is **immutable** — a correction
...
### S6 (in progress) — `0017`–`0019` (resources download · account prefs · support requests)

**S6 adds `0017`–`0019`** — the schema changes in the (mostly UI) private-platform sprint:
- **`0017_resources_download`** — the `storage.objects` SELECT policy on the private `resources` bucket (gated on `is_approved()`) + the `get_resource_download(p_id)` path-lookup RPC, so approved partners download templates via a server-issued signed URL (their own client, no secret key). Verify with `verification/0017_verify_TEST_db_only.sql` (role-simulated) + `0017_verify_PROD_safe_readonly.sql`.
- **`0018_account_prefs`** — `profiles.marketing_opt_in` + the owner-scoped `set_my_account` RPC (for `/account`).
- **`0019_support_requests`** — the `support_requests` table + `submit_support_request` (approved-only) + `admin_list_support_requests` (admin-only read), for `/support`.

Apply `0017`→`0019` in order on the **test** database first; **all three applied + role-matrix verified on test (`sdszcralogcrujtyghig`) 2026-06-19** (0017: anon/pending read nothing, approved can sign; 0018: a pending user updates only its own row, anon can't execute; 0019: approved insert ok, pending denied, admin reads / non-admin sees zero). **Production apply pending (owner)** — then run each `*_verify_PROD_safe_readonly.sql` there.
