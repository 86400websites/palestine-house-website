...
- [ ] 🔴 **Approval enforcement:** **every** platform RPC — read *and* write — checks `profiles.is_approved` **server-side**, and every gated page checks approval again before it renders. **State this as a blanket rule, never as a list of function names.** An enumeration silently becomes an allowlist: PP5's first draft of this bullet named the four read RPCs and thereby dropped the `submit_support_request` write and the `get_resource_download` signed-URL issuer out of scope, which an independent review caught (2026-08-14). The reads (`get_platform_sections`, `get_platform_topics`, `get_element`, `get_resources`), the write (`submit_support_request`) and the download issuer (`get_resource_download`) are **examples of the rule**; a new RPC is covered the day it is written. The **only** exception is an owner-scoped account/profile RPC — see the next bullet. An unapproved (pending) session can resolve only its own profile/approval status — never a guide body, a topic summary, a template row, or any of the dormant checklist/academy/session data. **The global search index inherits this** rather than restating it: it is built from those same gated reads, so a pending or anonymous caller gets an empty index (D-PP-j), and it carries no resource ids, storage paths or bucket names.
- [ ] 🔴 **`/account` is the one deliberate exception** — session-gated, no approval check, because a pending partner must be able to set their name and password while they wait. It reads and writes only the caller's own `profiles` row, through `set_my_account` (0018), which is scoped to `auth.uid()` and can never set `is_approved`. Any *other* gated route missing an approval check is a defect, not a second exception.
...
- [ ] 🔴 **Reference content never public:** any public projection/teaser exposes titles/overviews/metadata only, via anon-safe reads — never gated bodies or files. (The Experience live strip + the public `/live` listing were retired in LH1, 2026-07-10 — session data has no public surface at all, and PP5 deleted the members-only one too.)
- [ ] 🔴 **Templates & booklets:** the **88** templates live in a **private** Storage bucket; downloads only via **server-issued signed URLs** to authenticated, approved users. (The two public booklet PDFs — *The House Promise*, *Operating Model & Governance* — are the only public files, served as public lead magnets.) **Stage 4 addition (D-PP-f, 2026-08-12):** those rows are the live per-topic **templates grid**, selected by `element_id` + `doc_key IS NULL` + `code IS NOT NULL`. Any query or RPC that builds the grid **must also require `is_public = false` AND `storage_bucket = 'resources'`** — the three-column predicate alone would admit a future public upload into a partner-only surface (found by review, 2026-08-12). **The bucket half is now also enforced in the database** by `0029`'s `resources_private_bucket_shape` CHECK (`is_public OR storage_bucket = 'resources'`), so **D-PP-i is discharged** — the app-level predicate is defence in depth, not the only guard *(constraint verified on production 2026-08-21)*.
  - ✅ **CLOSED by PP6a's `0029` (D-PP-i), 2026-08-14 — and closed harder than it was owed.** The debt was to enforce the guard *in the RPC*. `0029` enforces it as **table CHECK constraints** instead, which hold against every future writer — a later RPC, a manual fix in the SQL Editor, an ingest script — not just callers who happen to use the right function. Four constraints, each verified against all 299 live rows (0 violations) before being added: **`resources_private_bucket_shape`** `check (is_public or storage_bucket = 'resources')` — every private file is in the private bucket; **`resources_private_needs_element`** `check (is_public or element_id is not null)` — every private file belongs to a focus area, which also closes the orphan-template case created by `resources_element_id_fkey ... ON DELETE SET NULL`; **`resources_guide_needs_element`** `check (doc_key is null or element_id is not null)` — the orphan-guide hole, which the partial unique index `resources_element_doc_key_ux (element_id, doc_key) WHERE doc_key IS NOT NULL` never covered, because without `NULLS NOT DISTINCT` it does not constrain rows with a NULL `element_id`; **`resources_private_needs_code`** `check (is_public or code is not null or doc_key is not null)` — every private file is badged, so it cannot silently drop out of the grid. ⚠️ **The bucket guard is deliberately CONDITIONAL, not blanket:** a flat `storage_bucket = 'resources'` CHECK would have **aborted `0029` at apply time**, because the two public booklet PDFs legitimately live in the `booklets` bucket. Proven on TEST by direct `INSERT` attacks that bypass every RPC — all four rejected with `check_violation`. `admin_register_resource_file` additionally does not *accept* `is_public` or `storage_bucket` as parameters at all: both are forced, so no caller can publish a template or point one at another bucket even by mistake.
  - 🔴 **Draft is a boundary at the BYTE layer too, not just the row layer** (added at the PP6a exit gate, 2026-08-14). Filtering the four read RPCs is not sufficient on its own: **the Storage API honours `storage.objects` policies directly**, and `0017` granted approved partners SELECT on *every* object in the private bucket with no link to `published`. A partner holding their own access token could therefore have listed the bucket and minted a signed URL for an unreleased focus area's files without any RPC being involved — Draft would have been real for rows and an illusion for bytes, exactly the failure `0029`'s own header claims to close. `0029` narrows `resources_bucket_select_approved` to `bucket_id = 'resources' AND is_approved() AND is_published_object(name)`. ⚠️ **`is_published_object()` must be `SECURITY DEFINER`** — a policy expression is evaluated as the *invoking* role, and `public.resources` is RLS default-deny, so an inline join matches nothing and denies **every** download instead. Both directions are regression-tested in `0029_verify_TEST_db_only.sql` §7b: Live must still be readable, Draft must not, an admin must keep unconditional read (or replace/delete break), anon gets nothing, and the two public booklets are untouched because they live in the separate public `booklets` bucket.
...
- [ ] 🔴 **Dormant tables are still governed tables.** PP5 deleted the Academy, the Live hub and the checklist UI, but `academy_modules`, `checklist_items`, `checklist_progress` and `programming_sessions` remain in the database until PP7's `0030`, with their RLS and gated RPCs intact — `programming_sessions` still reads only through the approved-gated `member_programming_sessions()` projection (0025, no anon path) and still takes owner-scoped writes. **Unreachable from the UI is not the same as unprotected**: do not relax a policy on one of these because "nothing calls it", and do not add a new caller. CSP allow-list stays extended **only** for the one chosen embed origin (YouTube), which PP6's CMS puts back to work.
- [ ] 🔴 **Admin routes** (`/admin/*`) verify the `admins` table server-side on every request — `is_approved` alone is not admin.
- [ ] 🔴 **A GATE IS A THROW, NOT AN AWAIT — and a parent layout is not a gate for its child** (added at PP8 8-k, 2026-08-20, after two independent reviews returned BLOCKING on the same defect). Next renders a page segment **in parallel with its layout**, so the segment's flight data can already be in the streamed response when the layout's `redirect()`/`notFound()` resolves. **Every gated page must run its own server-side check and short-circuit BEFORE it constructs any JSX** — before `await searchParams`, before any data read, as the first statement of the component. ⚠️ **Awaiting something is not a gate.** PP8 recorded the opposite ("awaiting `isAdmin()` makes the render depend on a server round-trip, which is what actually stops the segment streaming ahead of the gate") and it is **false**: `/admin/content/files` awaited `searchParams` **and two gated Supabase RPCs** — real network round trips — and still flushed its own heading and intro to an anonymous caller, as did `/admin/approvals`, `/pages`, `/admins` and `/focus-areas`. Only `notFound()`/`redirect()` **throwing before the JSX exists** closes it. The gated RPCs still fail closed, so what leaks is **structure, never data** — but the structure of an HQ tool is not public. This bullet exists because the previous wording ("verify … on every request") is satisfied by a layout alone, which is exactly the reading that produced the defect.
