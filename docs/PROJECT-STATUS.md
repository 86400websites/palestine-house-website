...
| **Next action** | Owner: **apply migration `0007` to the test DB then production and re-run verification** (independent review found applicants could pre-set `status='approved'`; `0007` restricts inserts to `pending`). Then review the S2 PR and merge (Preview shows only the unchanged site — S2 ships SQL, no UI). Then start **S3 — Auth complete** (Supabase `@supabase/ssr` clients + `middleware.ts` + login; Apply live = sign-up in 3c). Supabase env vars for Preview/Dev now point at the test project (`sdszcralogcrujtyghig`); production env vars already point at the prod project. (Dependabot #9–#12 already resolved — no open branches remain.) |
