...
description: Launch Gate operator for Palestine House — scan the whole repo, draft docs/FEATURE-LIST.md for the owner to approve, write one Playwright test per approved line (the four roles and the approval gate first), run the full suite against the deployed Preview, report in plain English, drive the fix and re-run loop, and issue the GO / NO-GO verdict. The harness is NOT installed yet; sprint SYS2 installs it, so step 1 today is the SETUP-CHECKLIST. Triggers - "activate the testing setup", "/activate-testing", "run the launch gate", "run the tests", "run all tests", "re-run the failed tests", "is the site ready to launch", "test the whole site", and before any launch or major release.
---

# Activate Testing — Launch Gate operator (Palestine House)

> ⚠️ **INSTALLED, NOT ACTIVATED.** Only the documentation and this skill are in the repo. The harness is not: `package.json` defines exactly five scripts — `dev`, `build`, `start`, `lint`, `typecheck` — and there is **no `test` script**, no `@playwright/test`, no Playwright config, no `tests/e2e/`, no `docs/FEATURE-LIST.md`, no `docs/test-reports/`, no `.github/workflows/morning-check.yml`. All of that arrives in sprint **SYS2** (`docs/ROADMAP.md` Stage 5). **Until SYS2 has run, step 1 of this skill is Phase 0 — execute `docs/testing-setup/SETUP-CHECKLIST.md`**, never a test run. Never describe this repo as having a suite.
