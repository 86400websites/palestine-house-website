...
export function SiteHeader() {
  ...
  return (
    <header className="phx-header" data-overlay={overlay ? "" : undefined}>
      ...
      {overlayRoute && (
        <noscript>
          <style>{`.phx-header[data-overlay]{background:rgba(250,246,238,.92);border-bottom-color:var(--line-warm);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px)}.phx-header[data-overlay] .phx-nav-link,.phx-header[data-overlay] .phx-signin,.phx-header[data-overlay] .phx-brand-word{color:var(--char-900)}.phx-header[data-overlay] .phx-brand-tagline{color:var(--char-900)}`}</style>
        </noscript>
      )}
