...
| Sprint | Phases | Scope & exit gate | Depends on |
|---|---|---|---|
| **S8 — Workspace Visual Polish** *(post-launch · build complete on `claude/sprint-s8-workspace-visual-polish`, pending merge)* | 8a shared shell + premium kit (reveals · tatreez dividers · paper washes · card-lift · empty/loading states) · 8b dashboard (3 states) · 8c plan + operate · 8d build tracker · 8e element page · 8f resources + resources/[category] + academy · 8g account + support + tools · **8i topic-page redesign + inline template downloads** · **8j public nav header above the workspace** · **8k retire the milestone gates concept site-wide (D-S8-c)** · 8h exit gate + review | Lift all ~12 gated pages to the **public-shell premium bar** using already-approved patterns + design-system tokens (no new design language, no new mockups); behaviour untouched. Grew on live owner feedback (topic-page redesign, public nav, gate removal — D-S8-b / D-S8-c). Exit: every workspace route reads premium + digestible at 320px + desktop + reduced-motion (owner Preview sign-off); typecheck/lint/build green; zero behaviour changes; independent Codex review (one finding fixed). Record: `docs/sprint-prompts/s8-workspace-visual-polish.md`. | S6 |
