...
| **S11 — Admin content management** *(post-launch · 🔵 BUILD COMPLETE on `claude/sprint-s11-admin-content-management`, pending owner PROD apply of 0023/0024 + merge)* | 11a admin-panel CRUD over the private-page content (the 30 element/topic bodies, academy videos, resources/templates) behind the existing `is_admin()` gate · 11b add / update / delete with RLS-safe writes · *(the old-S10 admin-management UI — add/remove admins — folds in here, to confirm at scope)* | The client fully self-manages all private-page content + resources without scripts — builds the graduated backlog `/admin/content`. Exit: an admin can add/update/delete content end-to-end behind the admin gate; writes RLS-safe; no content loss; versioned up + `.down.sql` + RLS in the PR (TEST-first, `WORKFLOW.md` §14). Brief: `docs/sprint-prompts/s11-admin-content-management.md`. | S5, S10 |
