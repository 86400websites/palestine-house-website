...
## Post-merge correction — PP1.1 (2026-08-11)

The owner adopted a **3-card topic model** the day after PP1 merged (decision **D-PP-c**, `PROJECT-STATUS.md` §5): each focus area shows **Overview / Simple guide / Template** — the Practical-checklist and What-to-watch-out-for cards are removed, the Template card is ONE owner-chosen file per focus area (via CMS v2, coming-soon until upload; the 297 uploaded coded files stay dormant as the picker's candidate list), and the 15 "More guides" extras are dropped entirely. Watch Video stays. The mockup (byte-identical to the Aug 6 export) still shows 4 cards — the owner's instruction supersedes it on this point only.

PP1's foundations were corrected **pre-PROD** on `claude/pp1-1-card-model` (0027/0028 never reached PROD, so the files were edited in place and TEST re-applied): `platform_extras` + `get_platform_extras` removed, `doc_key` CHECK → `('overview','guide','template')`, generator transform + regenerated spec/seed, verification scripts updated. **This record documents PP1 as shipped**; the corrected state lives in the PP1.1 row of `ROADMAP.md` Stage 4. Follow-up 1 above changes accordingly: the PP2-gate PROD apply uses the corrected files (EXPECT 5/10/33 · 297 coded · doc_keys 0 · no `platform_extras` · anon denied).

**TEST re-apply runbook — ✅ EXECUTED 2026-08-12** (via the `supabase-test` MCP in a fresh session; every EXPECT passed, plus a digest check proving the applied seed byte-matches `docs/workspace-spec.json` — full log in `pp1-1-card-model.md` → Checks & results). Kept here as the procedure of record: run each file whole, in order: ① `supabase/sql/migrations/0028_platform_seed.down.sql` ② `supabase/sql/migrations/0027_platform_ia.down.sql` ③ corrected `supabase/sql/migrations/0027_platform_ia.up.sql` ④ corrected `supabase/sql/migrations/0028_platform_seed.up.sql` ⑤ `supabase/sql/verification/0027_verify_TEST_db_only.sql` — every EXPECT must pass (three platform tables, RLS on, 0 policies · `extras_table = null`, `extras_rpc = 0` · 5/10/33 · coded 297, doc_keys 0 · `doc_key_check` lists overview/guide/template · role sims approved 5/33/299/297, pending 0s). TEST only; PROD never had 0027/0028. Then flip the PP1.1 rows in `ROADMAP.md` + `PROJECT-STATUS.md` to done.
