...
Legend: `[ ]` open · `[x]` fixed + re-verified (ticked in Step 3).

---

## HIGH

### `[ ]` S7-01 — Double chrome on every gated route except `/dashboard`
...
### `[ ]` S7-02 — `/account` "Change password" dead-ends (silently bounces to `/forgot-password`)
...
### `[ ]` S7-03 — Pending users' Support path dead-ends with no `/contact` fallback
...
### `[ ]` S7-04 — `/update-password` gives no feedback when it can't accept a submission
...
### `[ ]` S7-05 — Hardcoded "Stage · Plan & Prepare" top-bar label contradicts real stage
...
### `[ ]` S7-06 — `/apply` auto-signs-in existing accounts + email-enumeration oracle
...
### `[ ]` S7-07 — DB `youtube_url` rendered into `<a href>` with no scheme validation (latent stored-XSS)
...
### `[ ]` S7-08 — Authenticated users not redirected away from `/login` and `/apply`
...
### `[ ]` S7-09 — Literal `[contact email]` placeholder token shown on `/privacy` and `/terms`
...
### `[ ]` S7-11 — `/build` "Blocked?" note form can re-expand after a status round-trip (stale local state)
