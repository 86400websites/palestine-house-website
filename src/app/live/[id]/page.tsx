...
/* Watch view (/live/[id]) — the public watch page for one session (S9 9c).
   Reuses the cached getLiveSessions() feed (the one anon-callable RPC, 0013) and
   finds the row by id; an unknown id 404s via not-found.tsx. The video is a
   privacy-enhanced youtube-nocookie embed rebuilt from a validated 11-char id
   (youTubeEmbedUrl) — the single origin the CSP frame-src allows. A missing or
   non-YouTube link degrades to a graceful state, never a broken/blocked frame.
   New strings here follow brand voice (no locked copy exists for this view). */
...
export default async function WatchPage({
  ...
  return (
    <section className="ph-section-lg">
      <div className="ph-container live-watch">
