...
**4a — tests (SYS2).** No test runner, no test script, no test file, no `tests/` directory, and `@playwright/test` is not a dependency. SYS1 installed [`testing-setup/`](./testing-setup/) and the `/activate-testing` skill — the method, not the machine. SYS2 adds Playwright with `tests/e2e/`, generates one test per approved line of `docs/FEATURE-LIST.md` — including the negative access-gate tests for all four roles (anonymous · pending partner · approved partner · HQ admin) — and records the GO verdict in `docs/test-reports/`. *(All four of those paths arrive in SYS2; they do not exist yet.)* The CI test step is wired into `ci.yml` in that same PR.
