...
**Satisfied by SYS2 on 2026-08-22** — `tests/e2e/`, `pnpm run test:e2e` and `docs/test-reports/` all exist; the first full run was 136/136 green with the owner's signed GO. This historic block is kept for the record; the live status is the ✅ section above it.
...
- ✅ **Morning check LIVE (SYS2, 2026-08-22).** `.github/workflows/morning-check.yml` runs daily at 06:30 UTC against the live site as **Option A** — logged out, read-only, zero standing credentials (34 tests). Its one setup input is the `PRODUCTION_URL` repository **variable**. ⚠️ Its alarm channel is still unproven — see `PROJECT-STATUS.md` §7.
