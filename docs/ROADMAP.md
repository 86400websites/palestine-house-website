...
- [x] `/apply` — *live (S3c): sign-up → pending account + application record (zod-validated); single "Your name" field per D5 + an owner-approved Password field (§4). Rate-limit + Turnstile come in **S14** (post-launch hardening) — see `PROJECT-STATUS.md` §7*
...
- [ ] Hardening: Upstash rate limiting, Turnstile, security headers + CSP (embed origin only), fail-closed production forms — ***S14***
...
> Sequenced after go-live. **S8 (Workspace Visual Polish) and S9 (Live Programming) are merged.** **Replanned 2026-06-26 per owner direction (D-S10-b):** make the partner platform crystal-clear and premium (**S10**) → give the client full content control (**S11**) → wire the email automations as ready-to-switch-on placeholders (**S12**) → run a full multi-hat test-and-fix-everything pass (**S13**) → final hardening + relaunch on the custom domain with SEO (**S14**). One sprint at a time, same branch → PR → Preview → merge loop.
...
| **S13 — Full testing + fix-all → 0 bugs** *(post-launch)* | 13a multi-hat end-to-end walk (copywriting → UX → feature testing — every page, feature, CTA, link, a11y, security) · 13b **fix everything found, same sprint** · 13c re-verify | Cover everything in testing so there are no later surprises, then fix it all → a final, fully-working version with **0 errors / 0 bugs**. Absorbs the old S11 (journey QA) + S12 (fix-all). Exit: complete severity-ranked pass with every finding fixed + re-verified; production-quality, zero known bugs. Brief: `docs/sprint-prompts/s13-full-testing-fix-all.md`. | S10, S11, S12 |
| **S14 — Final hardening + relaunch** *(post-launch)* | 14a Upstash rate-limiting + Turnstile on public writes · 14b fail-closed verification in Production; CSP/headers verified live; secret-path threat-model pass · 14c custom domain + SEO · 14d final relaunch | Final hardening + go-live on the **custom domain** with SEO. Absorbs the old S12 hardening + final launch. Exit: full `SECURITY-CHECKLIST.md` §1–§15 pass; domain + SEO live; production re-smoke-tested; relaunched. Brief: `docs/sprint-prompts/s14-final-hardening-relaunch.md`. | S13 |
