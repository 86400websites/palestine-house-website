...
| **S8 — Email automations & lead magnets** | 8a Mailchimp: booklet capture (`lead-booklet-a/b`) + newsletter + apply tagging · 8b Resend: contact form + transactional (incl. the **approval-notification email deferred from S4**); verify sending domain (SPF/DKIM/DMARC) | Real deliveries verified on Preview + Production; keys server-only | S3 |
