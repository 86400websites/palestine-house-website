...
>
> **v3 (DR1, 2026-07-02): the public shell runs the warm, cultural, photography-led v3 system.** Its canonical token source is the code itself — `src/styles/globals.css` (`:root` v3 raw tokens) + `src/styles/v3.css` (the public-scoped semantic remap) — built from the owner's reference images in `docs/source-assets/design-refs/v3/`. The old mockups + bound design system (`/docs/page-designs/…`) and the Claude Design V2 project are **retired as design inputs for the public shell**; they are also retired for the **gated side**, which Stage 4 rebuilt on its own layer: `src/styles/workspace-v2.css`, scoped to `.pw-root`, sourced from the owner's final HTML prototype rather than from `page-designs/`. What still runs on the original green/paper system is **`/admin`** (`.adm-*`, §14 below) and any not-yet-migrated public layout. Three tiers, one token file: v3 values apply only inside `.ph-page` (+ `body:has(.ph-page)` for portalled UI), the v2 tokens live entirely inside `.pw-root`, and the legacy aliases serve what remains. Where this file and the shipped tokens disagree, **the code wins** — update this file.
...
- **Cards / surfaces:** consistent radius, subtle warm border, gentle lift on hover (≤1.02). *(The shared session card that `/live` and the Experience strip both used is gone: the public strip was retired at LH1 and the members-only hub at PP5.)*
...
- **Gated shell (workspace v2, Stage 4 — current since PP5, 2026-08-14):** a **fixed top header with five links (About · Setup · Operate · Program · Support)** + tooltips, an account button, a mobile panel, and a moss footer with the search CTA / Ask HQ / tatreez band / Arabic tagline. Its design layer is `src/styles/workspace-v2.css`, **entirely scoped to `.pw-root`** with its own token block — the gated side no longer shares the public shell's semantic aliases, and containment is proven on every sprint by a rule-level CSS bundle diff. Two pages in this shell are **not** in the mockup and were licensed as Claude's own design inside the same tokens: the **Simple guide reader** (PP4 — a 640px reading column at 18px, measured at ~72 characters per line) and **`/account`** (PP5 — three cards on the reader's column, and the system's first switch). Pending and declined states are `PwPendingState`, carrying the approved copy verbatim from the shell they replaced. *(Pre-Stage-4, for history: a persistent left sidebar — Welcome · Stages · Managing & Operating · Programming/Live Programming · Academy · Resources · House Applications · Account, Support pinned at the bottom — with pending items visible but locked. Deleted at PP5, along with its ~1,900 lines of CSS.)*
...
> **HISTORY, not a live brief (marked at PP5, 2026-08-14).** This section briefed S8's polish of the *pre-Stage-4* sidebar workspace, and every page it names was deleted at PP5. It is kept because `/admin` still runs on the legacy `.adm-*` layer described here, and because the reasoning — how the public shell's approved patterns were carried into a gated surface — is the same reasoning the v2 layer follows. **For the gated design system as it is now, read §8's "Gated shell" entry and `src/styles/workspace-v2.css`.** Nothing below should be treated as a specification for new work.
